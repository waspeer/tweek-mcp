...
import { RetryConfig, IdempotentMethod, HttpError } from './types.js'
  ...
  for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
    try {
      ...
      const shouldRetry = error instanceof Error &&
        ((error instanceof HttpError && isRetriableError(error.status)) ||
         ('status' in error && typeof error.status === 'number' && isRetriableError(error.status)))
      ...
      console.log(`Retrying request (attempt ${attempt + 1}/${config.maxAttempts}) after ${delay}ms`)
      await sleep(delay)
    }
  }
